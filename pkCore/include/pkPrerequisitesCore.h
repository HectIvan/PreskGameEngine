...
SPtr<Type>
pk_shared_ptr_new(Args&&... _args)
{
  return make_shared<Type>(std::forward<Args>(_args)...);
}

template<class Type, class AllocCategory = GenAlloc, class... Args>
SPtr<Type>
pk_unique_ptr_new(Args&&... _args)
{
  return make_unique<Type>(std::forward<Args>(_args)...);
}
