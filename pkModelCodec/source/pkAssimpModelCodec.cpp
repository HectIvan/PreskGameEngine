...
processMesh(const aiMesh* _mesh, const aiScene* _scene, const Matrix4 _transform)
{
  ...
  if (_mesh->mMaterialIndex >= 0) {
    ...
    if (diffCount < 1) {
      LOG_WARNING("Could not find albedo texture of material " + matName + ".",
                  __FILE__,
                  __LINE__);
    }
    ...
    for (uint32 i = 0; i < diffCount; ++i) {
      ...
      if (materialA->GetTexture(aiTextureType_DIFFUSE, i, &path) == AI_SUCCESS) {
        ...
        if (texture) {
          // log registry.
          LOG_REGISTER("Loaded albedo texture " + newPath.getFileName() +
                       " in material " + matName + ".",
                       __FILE__,
                       __LINE__);
        ...
        LOG_WARNING("Failed to load albedo texture" + filePath.toString() +
                    " in material " + matName + ".",
                    __FILE__,
                    __LINE__);
      }
    ...
    for (uint32 i = 0; i < normCount; ++i) {
      ...
      if (materialA->GetTexture(aiTextureType_HEIGHT, i, &path) == AI_SUCCESS) {
        ...
        if (texture) {
          // log registry.
          LOG_REGISTER("Loaded normal texture " + newPath.getFileName() +
                       " in material " + matName + ".",
                       __FILE__,
                       __LINE__);
        ...
        filePath = Path(path.C_Str()).getFileName();
        LOG_WARNING("Failed to load normal texture" + filePath.toString() +
                    " in material " + matName + ".",
                    __FILE__,
                    __LINE__);
      }
    ...
    for (uint32 i = 0; i < aoCount; ++i) {
      ...
      if (materialA->GetTexture(aiTextureType_AMBIENT, i, &path) == AI_SUCCESS) {
        ...
        if (texture) {
          // log registry.
          LOG_REGISTER("Loaded ambient occlussion texture " + newPath.getFileName() +
                       " in material " + matName + ".",
                       __FILE__,
                       __LINE__);
        ...
        filePath = Path(path.C_Str()).getFileName();
        LOG_WARNING("Failed to load ambient occlussion texture" + filePath.toString() +
                    " in material " + matName + ".",
                    __FILE__,
                    __LINE__);
    }
    ...
    for (uint32 i = 0; i < metallicCount; ++i) {
      ...
      if (materialA->GetTexture(aiTextureType_METALNESS, i, &path) == AI_SUCCESS) {
        ...
        if (texture) {
          // log registry.
          LOG_REGISTER("Loaded metallic texture " + newPath.getFileName() +
                       " in material " + matName + ".",
                       __FILE__,
                       __LINE__);
        ...
        filePath = Path(path.C_Str()).getFileName();
        LOG_WARNING("Failed to load metallic texture" + filePath.toString() +
                    " in material " + matName + ".",
                    __FILE__,
                    __LINE__);
      }
    ...
    for (uint32 i = 0; i < roughnessCount; ++i) {
      ...
      if (materialA->GetTexture(aiTextureType_REFLECTION, i, &path) == AI_SUCCESS) {
        ...
        if (texture) {
          // log registry.
          LOG_REGISTER("Loaded roughness texture " + newPath.getFileName() +
                       " in material " + matName + ".",
                       __FILE__,
                       __LINE__);
        ...
        filePath = Path(path.C_Str()).getFileName();
        LOG_WARNING("Failed to load roughness texture" + filePath.toString() +
                    " in material " + matName + ".",
                    __FILE__,
                    __LINE__);
      }
    ...
    for (uint32 i = 0; i < emissiveCount; ++i) {
      ...
      if (materialA->GetTexture(aiTextureType_EMISSIVE, i, &path) == AI_SUCCESS) {
        ...
        filePath = Path(path.C_Str()).getFileName();
        LOG_WARNING("Failed to load emissive texture" + filePath.toString() +
                    " in material " + matName + ".",
                    __FILE__,
                    __LINE__);
      }
