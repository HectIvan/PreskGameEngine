...
InverseKinematics::fabrik(Vector3 _target)
{
  ...
  float dist = _target.distanceTo(m_bones[0]->actorIni->m_transform.getTranslation3());
  // if the distance calculated is more than the max distance of the arm
  if (dist > maxDistance) {
    // set the target to the max direction
    Vector3 dir = (m_bones[0]->iniPos -
                   getLastBone()->actorIni->m_transform.getTranslation3()).normalized();
    dir *= maxDistance;
    _target = dir;
  ...
  for (uint32 i = m_bones.size() - 1; i > 0; --i) {
    if (i > 0) {
      m_bones[i]->actorIni->m_transform.setTranslation(_target);
      _target = (_target - m_bones[i - 1]->actorIni->m_transform.getTranslation3()).normalized();
      _target *= m_bones[i]->distance;
    }
  }
}
}