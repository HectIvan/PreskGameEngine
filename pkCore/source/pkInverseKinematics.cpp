...
InverseKinematics::insertNodeLocal(const Vector3& _position, const SPtr<Actor>& _pActor)
{
  ...
  if (!m_bones.empty()) {
    ...
    lastBone->distance = Math::distance(lastBone->actorIni->getPosition3(), _position);
  }
...
void
InverseKinematics::deleteNode(const uint32& _index)
{
...
void
InverseKinematics::deleteBone(const uint32& _index)
{
  // check if index is in range.
...
void
InverseKinematics::fabrik(const Vector3& _target)
{
  const Vector3 rootPos = m_bones[0]->actorIni->getPosition3();
  ...
  if (dist > maxDistance) {
    ...
    const Vector3 direction = (_target - rootPos).normalized();
    // set all joints to one direction.
    for (uint32 i = 1; i < m_bones.size(); ++i) {
      const Vector3 prevPos = m_bones[i - 1]->actorIni->getPosition3();
      const Vector3 newPos = prevPos + direction * m_bones[i - 1]->distance;
  ...
  // if the target is inside the range of the arm.
    ...
    for (uint32 i = bCount - 1; i > 0; --i)
    {
      ...
      const Vector3 directionNorm = (m_bones[i - 1]->actorIni->getPosition3() - target).normalized();
    ...
    {
      const Vector3 nextPos = m_bones[i]->actorIni->getPosition3();
      const Vector3 prevPos = m_bones[i - 1]->actorIni->getPosition3();
      const Vector3 directionNorm = (nextPos - prevPos).normalized();
      // distance from this bone to the previous bone.
      dist = m_bones[i - 1]->distance;
      // set the new target for the next bone move.
